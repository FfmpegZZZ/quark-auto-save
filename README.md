...
程序默认会跳过已存在的重名文件，以避免重复转存浪费空间。

**普通模式（默认）**：
- 检测到目标位置已存在同名文件时，跳过转存该文件
- 可配合"忽略后缀"选项，将 `01.mp4` 和 `01.mkv` 视为同一文件

**覆盖模式**：
- 即使目标位置存在同名文件也要转存
- 转存前会自动删除原有的重名文件，实现覆盖效果
- 删除后会等待60秒确保夸克服务器状态更新，然后验证删除结果
- 转存时采用重试机制，最多重试3次以提高成功率
- 适用于需要更新已转存文件的场景

> [!WARNING]
> ⚠️ 覆盖模式会删除目标位置的原有文件，请谨慎使用，建议先在测试目录验证效果。

> [!TIP]
> 如果覆盖模式下仍然出现转存失败的情况，请检查运行日志。程序会详细记录删除和转存的每个步骤，帮助定位问题。
